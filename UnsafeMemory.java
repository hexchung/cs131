...
class UnsafeMemory {
    ...
    public static void main(String args[]) {
		...
		try {
			...
			int[] value1 = new int[args.length - 4];

			for (int i = 4; i < args.length; i++) {
				value[i - 4] = (byte) parseInt(args[i], 0, maxval);
				value1[i - 4] = (int) parseInt(args[i], 0, maxval);
			}

			byte[] stateArg = value.clone();
			int[] stateArg1 = value1.clone();
			State s;
			...
			if (args[0].equals("Null")) {
				...
				s = new GetNSetState(stateArg1, maxval);
				dowork(nThreads, nTransitions, s);
				test1(value1, s.currentarr(), maxval);
				System.exit(0);
	...
	private static void test1(int[] input, AtomicIntegerArray output,
                              byte maxval)  {
		...
		for (int i = 0; i < input.length; i++) {
			...
			osum += output.get(i);
			if (output.get(i) < 0)
				error("negative output", output.get(i), 0);
			if (output.get(i) > maxval)
				error("output too large", output.get(i), maxval);
		}
		if (isum != osum)
			error("sum mismatch", isum, osum);
	}
